...
const DatabaseManager30Min = require('./utils/databaseManager30Min');
...
const dbManager = new DatabaseManager30Min();
...
const traderInsights = [
    { question: "What's the short-term price outlook for the IE market?", answer: "High wind generation is putting downward pressure on prices, but watch for evening demand spikes." },
    { question: "Is there an arbitrage opportunity between GB and IE?", answer: "Currently minimal. Interconnector is near capacity and price differential is low." },
    { question: "How reliable are the solar forecasts for GB?", answer: "Partly cloudy conditions are making forecasts less certain. Expect some deviation from actuals." },
    { question: "Should we be concerned about the load forecast accuracy in CY?", answer: "It's within normal parameters, but tourist season is causing some unusual demand patterns." },
    { question: "What's the biggest risk factor right now?", answer: "Unplanned generator outages. Keep an eye on real-time generation data." }
];
...
async function initializeData() {
    try {
        ...
        try {
            ...
            console.log('⏳ This will take a few minutes for 30 days of data...\n');
            
            // Fetch 30 days of historical data (can be increased to 365 for full year)
            dbData = await entsoeDataManager.fetchHistoricalData(30);
            
            if (dbData.length > 0) {
                currentIndex = 0;
                dataSource = 'entsoe-historical';
                console.log('✅ ENTSO-E data initialized successfully');
                const cacheInfo = entsoeDataManager.getCacheInfo();
                if (cacheInfo.dateRange.start) {
                    console.log(`📅 Data range: ${cacheInfo.dateRange.start} to ${cacheInfo.dateRange.end}`);
                    console.log(`📊 Total data points: ${cacheInfo.dataPoints}\n`);
                }
                ...
                throw new Error('ENTSO-E returned 0 data points');
            }
        } catch (entsoeError) {
    ...
    realtimeRefreshInterval = setInterval(async () => {
        try {
            ...
            if (freshData.length > 0) {
                ...
                const existingData = entsoeDataManager.getAllData();
                
                // Find the cutoff point (remove overlapping old data)
                const cutoffTime = freshData[0].timestamp;
                const filteredOldData = existingData.filter(point => point.timestamp < cutoffTime);
                
                // Combine old + new
                dbData = [...filteredOldData, ...freshData];
                entsoeDataManager.cache = dbData;
                
                console.log(`✅ Real-time data refreshed: ${freshData.length} new points added`);
...
function getCurrentDataPoint() {
    ...
    while (attempts < 100) { // Prevent infinite loop
        ...
        // Check if this record has some actual data (not all null)
        if (record.cy_load_actual !== null || 
...
function startStreaming(intervalMs = 2000) {
    ...
    streamingInterval = setInterval(async () => {
        try {
            ...
            const countResult = await dbManager.pool.query('SELECT COUNT(*) FROM energy_trading_30min');
            const totalRecords = parseInt(countResult.rows[0].count);
            
            // Query records with offset for progression
            const query = `
                SELECT * FROM energy_trading_30min 
                ORDER BY timestamp ASC 
                LIMIT 1 OFFSET $1
            `;
            ...
            if (currentIndex % 15 === 0) { // Every 15 data points
                const news = marketNews[Math.floor(Math.random() * marketNews.length)];
                io.emit('marketNews', news);
            }
            if (currentIndex % 25 === 0) { // Every 25 data points
                const insight = traderInsights[Math.floor(Math.random() * traderInsights.length)];
            ...
            if (currentIndex % 10 === 0) { // Every 10th record
                ...
                console.log(`  Renewable data:`, {
                    gb_total_wind: processedData.countries.GB?.total_wind,
                    ie_total_wind: processedData.countries.IE?.total_wind,
                    cy_wind: processedData.countries.CY?.wind_onshore
                });
                console.log(`  Price data:`, {
                    ie_sem_price: processedData.countries.IE?.sem_price_day_ahead
                });
...
io.on('connection', (socket) => {
    ...
    // Send latest data point to new client
    if (dbData.length > 0) {
        const currentData = processDataPoint(getCurrentDataPoint());
        socket.emit('energyData', {
            ...currentData,
            index: currentIndex,
            total: dbData.length,
            progress: ((currentIndex / dbData.length) * 100).toFixed(2)
        });
    }
    
    // Handle client commands
    socket.on('startStream', (data) => {
        const interval = data?.interval || 2000;
        startStreaming(interval);
        io.emit('streamStatus', { isStreaming: true });
    });
    
    socket.on('stopStream', () => {
        stopStreaming();
        io.emit('streamStatus', { isStreaming: false });
    });
    
    socket.on('resetStream', () => {
        currentIndex = 0;
        console.log('Stream reset to beginning');
        io.emit('streamReset', { currentIndex: 0 });
    });
    
    socket.on('changeDataset', async (data) => {
        const { dataset } = data;
        const validDatasets = [
        ...
        if (validDatasets.includes(dataset)) {
            ...
            try {
                dbData = await loadCSVData(selectedDataset);
                currentIndex = 0;
                dataSource = 'csv';
                io.emit('datasetChanged', { 
                ...
                socket.emit('error', { message: 'Failed to load dataset' });
            }
        }
    });
